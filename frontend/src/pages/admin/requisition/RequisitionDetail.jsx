...
export default function RequisitionDetail() {
  ...
  const totalCost = requisition.items?.reduce((sum, item) =>
    sum + (item.quantity * (item.costPrice ?? 0)), 0) ?? 0;
  const hasCostPrices = requisition.items?.some(i => i.costPrice != null);
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <div className="flex items-start justify-between mb-5">
          <div>
            <h1 className="text-xl font-extrabold text-slate-800">Requisition Details</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-5 border-t border-slate-100">
          <div>
            <p className="text-xs text-slate-400 mb-1 flex items-center gap-1"><User size={11} /> Employee</p>
            <p className="font-semibold text-slate-700 text-sm">
              {requisition.employee?.firstName} {requisition.employee?.lastName}
            </p>
            <p className="text-xs text-slate-400">{requisition.employee?.position}</p>
          </div>
          <div>
            ...
            return (
              <div key={item.id} className="border border-slate-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-3 gap-3 text-xs">
                  ...
                  <div className="bg-slate-50 rounded-lg p-2.5">
                    <p className="text-slate-400 mb-0.5">Cost Price</p>
                    <p className="font-bold text-slate-700">
                      {item.costPrice != null ? fmt(item.costPrice) : <span className="text-slate-300">—</span>}
                    </p>
                  </div>
                </div>
                ...
                {item.receivingLogs?.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-slate-100">
                    <p className="text-xs font-semibold text-slate-400 mb-2 flex items-center gap-1">
                      <History size={11} /> Receiving History
                    </p>
                    <div className="space-y-1.5">
                      {item.receivingLogs.map((log) => (
                        <div key={log.id} className="flex flex-wrap items-center gap-2 text-xs text-slate-500 bg-slate-50 rounded-lg px-3 py-1.5">
                          ...
                          <span>by <strong>{log.receivedByName ?? log.receivedById}</strong></span>
                          <span className="text-slate-300">·</span>
                          <span>{new Date(log.receivedAt).toLocaleString()}</span>
                          {log.note && (
                            <span className="italic text-slate-400 ml-auto">"{log.note}"</span>
                          )}
                        </div>
                      ))}
                    </div>
      ...
      {hasCostPrices && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex items-center justify-between">
          <p className="font-semibold text-slate-700">Estimated Total Cost</p>
          <p className="text-xl font-extrabold text-slate-800">{fmt(totalCost)}</p>
        </div>
      )}

      {/* Action buttons */}
      {(requisition.status === 'PENDING' || requisition.status === 'APPROVED' || requisition.status === 'PARTIALLY_RECEIVED') && (
        <div className="flex flex-wrap gap-3">
          {requisition.status === 'PENDING' && (
            <>
              <button
                onClick={() => navigate(`/admin/requisition-management/approve/${id}`)}
          ...
          {(requisition.status === 'APPROVED' || requisition.status === 'PARTIALLY_RECEIVED') && (
            <button
              onClick={() => navigate(`/admin/requisition-management/receive/${id}`)}
