...
import {
  ArrowLeft, User, Phone, Briefcase, Camera, Save,
...
function FileUploadZone({ label, hint, accept, icon: Icon, file, preview, onFile, onClear, isImage, existingUrl, existingName }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      {file ? (
        <div style={{
          ...
          background: 'var(--bg-sunk)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)',
        }}>
          {isImage && preview ? (
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 10, color: 'var(--fg-subtle)' }}>{(file.size / 1024).toFixed(0)} KB · new file</div>
          </div>
      ...
      /* Existing file from server */
        ...
        <div style={{
          ...
          {isImage ? (
            <img src={existingUrl} alt="" style={{ width: 40, height: 40, borderRadius: 'var(--r-sm)', objectFit: 'cover', flexShrink: 0 }} />
          ) : (
            <div style={{ width: 40, height: 40, borderRadius: 'var(--r-sm)', background: 'color-mix(in srgb, var(--accent) 12%, transparent)', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
              <Icon size={18} style={{ color: 'var(--accent)' }} />
            </div>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 10, color: 'var(--fg-subtle)' }}>Current file</div>
          </div>
          <a href={existingUrl} target="_blank" rel="noreferrer" className="icon-btn" title="Open">
            <ExternalLink size={12} />
          </a>
          <button type="button" className="stoq-btn stoq-btn--sm" onClick={() => ref.current?.click()} style={{ flexShrink: 0 }}>
            Replace
          </button>
        </div>
    ...
    employeeService.getEmployee(id)
      .then(data => {
        ...
        setExisting({
          ...
          cvDocument:         fileUrl(data.cvDocument),
          supportingDocument: fileUrl(data.supportingDocument),
        });
        if (data.profilePicture) setImagePreview(fileUrl(data.profilePicture));
      })
    ...
    <div style={{ padding: '20px 24px 40px' }}>
      ...
      <div className="page-head">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <h1>Edit Employee</h1>
            <div className="page-head__sub">Update staff profile and documents</div>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 14, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="stoq-panel">
            <div className="stoq-panel__head">
              ...
              <span style={{ fontSize: 11, color: 'var(--fg-subtle)' }}>Upload or replace employee documents</span>
            </div>
            <div style={{ padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
              <FileUploadZone
                ...
                existingUrl={existing.idCardImage}
                existingName={basename(existing.idCardImage)}
              ...
              <FileUploadZone
                ...
                file={cvFile}
                preview={null}
                existingUrl={existing.cvDocument}
                existingName={basename(existing.cvDocument)}
              ...
              <FileUploadZone
                ...
                file={supportingFile}
                preview={null}
                existingUrl={existing.supportingDocument}
                existingName={basename(existing.supportingDocument)}
        ...
        <div className="stoq-panel">
          ...
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            ...
            <input type="file" ref={photoRef} accept="image/*" onChange={e => {
